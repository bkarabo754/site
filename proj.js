import React from 'react'
import { motion } from 'framer-motion'
import { fadeIn } from '../variants'
import Image1 from '../assets/image1.png'
import Image2 from '../assets/image2.png'
import Image3 from '../assets/image4.png'

const Work = () => {
  return (
    <section className="section bg-[#0F1624]" id="work">
      <div className="container mx-auto">
        <div className='flex flex-col lg:flex-row gap-x-8'>
            <div className='flex-1 flex flex-col gap-y-12 mb-10 lg:mb-0'>
                <div>
                    <h2 className="h2 leading-tight text-accent">My Latest <br /> Work.</h2>
                    <p className="max-w-sm mb-16">Have a look at these cool projects I built using React, NextJS, Tailwind CSS, Framer Motion and other cool libraries.</p>
                    <button className="btn btn-sm">View all projects</button>
                </div>
                <div className='group relative overflow-hidden border-2 border-white/50 rounded-xl'>
                    <div className='group-hover:bg-black/70 w-full h-full absolute z-40 transition-all duration-300 cursor-pointer'></div>
                    <img className="group-hover:scale-125 transition-all duration-500" src={Image1} alt="" />
                    <div className="absolute -bottom-full left-12 group-hover:bottom-24 transition-all duration-500 z-50">
                    <span className="text-gradient">UI/UX Design</span>
                    </div>
                    <div className="absolute -bottom-full left-12 group-hover:bottom-14 transition-all duration-700 z-50">
                        <span className="text-3xl text-white">Project Title</span>
                    </div>
                </div>
            </div>
            
            <div className="flex-1">2</div>
        </div>
      </div>
    </section>
  )
}

export default Work
